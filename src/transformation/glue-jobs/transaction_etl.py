...
def clean_transaction_data(df):
    ...
    df = df.withColumn(
        "transaction_date", F.to_timestamp("transaction_date", "yyyy-MM-dd HH:mm:ss")
    )
    ...
    df = df.withColumn("total_amount_calculated", F.col("quantity") * F.col("price"))

    df = df.withColumn(
        "total_amount",
        F.when(
            F.col("total_amount").isNull()
            | (F.abs(F.col("total_amount") - F.col("total_amount_calculated")) > 0.01),
...
def main():
    ...
    try:
        ...
        df_final.write.mode("overwrite").partitionBy("year", "month", "day").parquet(
