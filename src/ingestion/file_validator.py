...
SCHEMAS = {
    ...
    'clickstream': {
        'required_fields': [
            'session_id', 'customer_id', 'event_type', 'page', 'timestamp', 'device'
        ],
        'format': 'json'
    },
    'reviews': {
...
class FileValidator:
    ...
    def validate(self) -> Dict:
        ...
        if file_size > 1024 * 1024 * 1024:
            result['warnings'].append('File size exceeds 1GB, will use Glue for processing')
    ...
    def _validate_format(self) -> Dict:
        ...
        if expected_format == 'csv' and not self.key.endswith('.csv'):
            ...
            result['valid'] = False
            result['errors'].append(f'Expected JSON file, got {self.key.split(".")[-1]}')
