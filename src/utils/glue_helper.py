...
class GlueHelper:
    ...
    def start_job_run(
        self, job_name: str, arguments: Optional[Dict] = None
    ) -> Optional[str]:
        """
    ...
    def wait_for_job_run(
        self, job_name: str, run_id: str, max_wait_seconds: int = 3600
    ) -> str:
        """
    ...
    def wait_for_crawler(self, crawler_name: str, max_wait_seconds: int = 1800) -> str:
        """
    ...
    def get_partitions(self, database_name: str, table_name: str) -> List[Dict]:
        ...
        try:
            ...
            logger.info(
                f"Found {len(partitions)} partitions for {database_name}.{table_name}"
            )
            return partitions
        except ClientError as e:
    ...
    def create_partition(
        ...
        try:
            ...
            logger.info(
                f"Created partition {partition_values} for {database_name}.{table_name}"
            )
            return True
        except ClientError as e:
